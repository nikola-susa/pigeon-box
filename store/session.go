...
func (s *Store) CreateSession(session model.Session) (*int, error) {
	row := s.db.QueryRowContext(
		context.Background(),
		`INSERT INTO session (user_id, thread_id, expires_at) VALUES ($1, $2, $3) RETURNING id`,
		session.UserID,
		session.ThreadID,
		session.ExpiresAt,
	)
...
func (s *Store) GetSession(id int) (*model.Session, error) {
	...
	err := s.db.Get(&session, `SELECT * FROM session WHERE id = $1`, id)
	if err != nil {
...
func (s *Store) GetSessionByUserID(userID int) (*model.Session, error) {
	var session model.Session
	err := s.db.Get(&session, `SELECT * FROM session WHERE user_id = $1`, userID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (s *Store) DeleteSessionByUserID(userID int) error {
	_, err := s.db.Exec(`DELETE FROM session WHERE user_id = $1`, userID)
	if err != nil {
